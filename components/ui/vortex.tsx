...
export const Vortex = (props: VortexProps) => {
	...
	const setup = () => {
		...
		if (canvas && ctx) {
			...
			let baseHue = props.baseHue || 220;
			let baseSpeed = props.baseSpeed || 0.1;
			let rangeSpeed = props.rangeSpeed || 1;
			let baseRadius = props.baseRadius || 3;
			let rangeRadius = props.rangeRadius || 2;
	...
	const draw = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, particleProps: Float32Array, baseHue: number, baseSpeed: number, rangeSpeed: number, baseRadius: number, rangeRadius: number) => {
		...
		window.requestAnimationFrame(() => draw(canvas, ctx, particleProps, baseHue, baseSpeed, rangeSpeed, baseRadius, rangeRadius));
	};
	...
	const updateParticle = (i: number, ctx: CanvasRenderingContext2D) => {
		...
		(checkBounds(x, y, canvas) || life > ttl) && initParticle(particleProps, i);
	};

...
export default function AnalysisItem({ id, title, description, performanceIncrease, seoImprovement, mobileScore, imageUrl, tags }: AnalysisItemProps) {
	...
	return (
		<div className="bg-zinc-50 dark:bg-black border-t border-neutral-200 dark:border-neutral-800 py-16">
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="lg:flex lg:items-center lg:justify-between">
					<div className="lg:w-1/2 pr-8">
						<div className="flex items-center justify-between mb-4">
							<h2 className="text-3xl font-extrabold text-black dark:text-white sm:text-4xl">{title}</h2>
							<span className="text-2xl font-bold text-neutral-400 dark:text-neutral-600">#{id}</span>
						</div>
						<p className="mt-3 text-lg text-neutral-600 dark:text-neutral-400 mb-6">{description}</p>
						<div className="grid grid-cols-3 gap-4 mb-6">
							<div className="flex flex-col items-center justify-center p-4 bg-neutral-100 dark:bg-neutral-900 rounded-lg">
								...
								<span className="text-sm text-neutral-600 dark:text-neutral-400">Faster</span>
							</div>
							<div className="flex flex-col items-center justify-center p-4 bg-neutral-100 dark:bg-neutral-900 rounded-lg">
								...
								<span className="text-sm text-neutral-600 dark:text-neutral-400">SEO Boost</span>
							</div>
							<div className="flex flex-col items-center justify-center p-4 bg-neutral-100 dark:bg-neutral-900 rounded-lg">
								...
								<span className="text-sm text-neutral-600 dark:text-neutral-400">Mobile Score</span>
							</div>
						</div>
						<div className="flex flex-wrap gap-2 mb-6">
							{tags.map((tag) => (
								<span key={tag} className="px-3 py-1 text-sm font-medium bg-neutral-200 text-black rounded-full dark:bg-neutral-800 dark:text-white">
									{tag}
								</span>
							))}
						</div>
						<a href={`/analysis/${id}`} className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-black hover:bg-neutral-800 dark:text-black dark:bg-zinc-50 dark:hover:bg-neutral-200 transition duration-150 ease-in-out">
							View Full Analysis

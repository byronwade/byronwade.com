...
const product = {
	...
	features: ["Responsive design for all devices", "Dark and light mode support", "Over 50 pre-built components", "Advanced charting libraries integration", "Seamless data fetching with React Query", "Fully customizable with Tailwind CSS"],
	images: ["https://placehold.co/800x1200", "https://placehold.co/800x1200", "https://placehold.co/800x1200"],
	reviews: [
		...
		<form onSubmit={handleSubmit} className="space-y-4">
			<div>
				...
				<div className="flex items-center space-x-1 mt-1">
					{[1, 2, 3, 4, 5].map((value) => (
						<button key={value} type="button" onClick={() => setRating(value)} className={`${rating >= value ? "text-yellow-400" : "text-muted-foreground"}`}>
			...
			<div>
				...
				<Textarea id="comment" value={comment} onChange={(e) => setComment(e.target.value)} className="mt-1" rows={4} />
			</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
					<div className="lg:sticky lg:top-12 space-y-8">
						<div className="relative aspect-[4/3] overflow-hidden rounded-lg">
							...
							<Button variant="outline" size="icon" className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-background/80 hover:bg-background" onClick={nextImage}>
								<ChevronRight className="h-6 w-6" />
							</Button>
						</div>
						<div className="flex justify-center space-x-2">
							{product.images.map((_, index) => (
								<Button key={index} variant={index === currentImageIndex ? "default" : "outline"} size="sm" onClick={() => setCurrentImageIndex(index)}>
									{index + 1}
								</Button>
							))}
						</div>
					...
					<div className="space-y-8">
						<div>
							<Badge className="mb-2 bg-primary text-primary-foreground">{product.type}</Badge>
								...
								<div className="space-y-4">
									{product.additionalServices.map((service) => (
										<div key={service.id} className="flex items-start space-x-3">
											<Checkbox id={service.id} checked={selectedServices.includes(service.id)} onCheckedChange={() => handleServiceChange(service.id)} />
											<div className="grid gap-1.5 leading-none">
												...
												{service.id === "setup" && (
													<p className="text-sm text-primary">
														Or{" "}
														<a href="/setup-instructions" className="underline">
															try it alone first for free
														</a>
													</p>
												)}
												{service.recurring && <p className="text-xs text-destructive">This is a recurring monthly charge.</p>}
											</div>
										</div>
									))}
								</div>
